...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6"><h1 class="m-0">Tambah Pengeluaran</h1></div>
            </div>
        </div>
    </div>
                ...
                <div class="col-md-12">
                    <form method="POST" action="{{ route('store.expense') }}">
                        ...
                        <div class="text-center">
                            <a href="javascript:void(0);" id="add-order" class="btn btn-success mb-3">Tambah Pengeluaran</a>
                        </div>
...
<script>
    ...
    addOrderButton.addEventListener('click', () => {
        ...
        newCard.innerHTML = `
            <div class="card-body">
                <!-- Menu -->
                <div class="form-group">
                    <label for="menu-select">Kategori</label>
                    <select class="form-control menu-select" name="expenses_id[]" required>
                        <option value="">-- Pilih Menu --</option>
                        @foreach ($expenses as $expense)
                            <option value="{{ $expense->id }}" data-price="{{ $expense->price }}" data-category="{{ $expense->item }}">
                                {{ $expense->item }} - Rp{{ number_format($expense->price, 0, ',', '.') }}
                            </option>
                        @endforeach
                        <option value="Tip Karyawan" data-price="0" data-category="Tip Karyawan">Tip Karyawan</option>
                    </select>
                </div>
    ...
    function updateAmount(card) {
        ...
        if (selected.value === 'Tip Karyawan') {
            // Logika untuk Tip
            descriptionTextarea.value = "Tip Karyawan"; // Set description as "Tip"
